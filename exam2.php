    ...
    if (file_exists($filename)) {
        ...
        foreach ($lines as $line) {
            ...
            if (preg_match('/^\d+\.\s+(.*?)$/', $line, $matches)) {
                ...
                // Adding options for uppercase letters
                $options[] = trim($matches[2]);
            } elseif (preg_match('/^Correct answer:\s+([A-D])\.\s*(.*)$/', $line, $matches)) {
                // Capturing the correct answer
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $examGiven = 'HTML/CSS/JS'; // Make sure this reflects the correct exam subject
