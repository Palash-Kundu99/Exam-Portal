...
// Load questions from qus.txt only if not already set in session
if (!isset($_SESSION['question'])) {
    $questions = [];
    $filename = 'question/PHPJSSQL.txt'; 
    ...
    if (file_exists($filename)) {
        ...
        foreach ($lines as $line) {
            ...
            } elseif (preg_match('/^Correct answer:\s+(.*)$/', $line, $matches)) {
                // Capture the correct answer
                $correctAnswer = trim(substr($matches[1], 0, 1)); // Only take A, B, C, or D
            }
        }
    ...
    shuffle($questions);
    $_SESSION['question'] = array_slice($questions, 0, 25);
}

// Retrieve questions from session
$selectedQuestions = $_SESSION['question'];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    foreach ($selectedQuestions as $index => $question) {
        $correctAnswer = trim($question[5]); // Correct answer is the last part
    ...
    // Store the score and exam given in the database
    $email = $_SESSION['email'];
    $examGiven = 'PHP'; // This should be dynamically set based on the exam taken (e.g., "PHP", "HTML", etc.)
    
    $stmt = $mysqli->prepare("UPDATE students SET exam_given = ?, score = ? WHERE email = ?");
    $stmt->bind_param("sis", $examGiven, $percentageScore, $email);
    $stmt->execute();
    $stmt->close();

    // Check if the score is 80 or more for certificate download
    $certificatePath = '';
    if ($percentageScore >= 80) {
        $certificatePath = 'certificates/certificate_' . uniqid() . '.pdf'; // Example path for certificate
        // Logic to generate/save the certificate PDF would go here (not included for brevity)
    }

    // Destroy the session
    session_destroy(); // This will end the current session

    // Redirect to the dashboard page with score and certificate info
